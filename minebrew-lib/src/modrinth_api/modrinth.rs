...
impl<'a> ToRequest for ListVersionReq<'a> {
    fn to_req(&self) -> String {
        let mut req = format!("{}/project/{}/version", BASE_REQUEST, self.id);
        ...
        if let Some(loaders) = &self.loaders {
            write!(&mut req, "{}loaders=[", param_prefix).unwrap();
            param_prefix = '&';
        ...
        if let Some(versions) = &self.game_versions {
            write!(&mut req, "{}game_versions=[", param_prefix).unwrap();
            param_prefix = '&';
...
// default new modrinth instance
impl Default for Modrinth<Empty> {
    fn default() -> Self {
        Self {
...
impl Modrinth<Empty> { /// Creates a new Modrinth instance with an empty Request, calling `get()` on this 
...
impl <'a> Modrinth<ProjectReq<'a>> {
    ...
    pub async fn get(&mut self) -> anyhow::Result<Project> {
        let proj: Project = self.make_request().await?.json().await?;
        Ok(proj)
    }
}
...
impl<R: ToRequest> Modrinth<R> {
    ...
    fn make_request(&mut self) -> impl futures::Future<Output=reqwest::Result<Response>> {
...
mod tests {
    ...
    #[track_caller]
    fn check_to_req<R: ToRequest>(req_type: R, expected: String) {
    ...
    fn empty_req() {
        ...
        check_to_req(Modrinth::new().req_type, expected.into());
    }
    ...
    fn basic_search() {
        ...
        check_to_req(api_call, expected.into());
    }
