...
async fn install(mut opts: Options) {
    ...
    let resps = mbrew.search(&searches).await.unwrap();

    // filter out and choose search results
    let results: Vec<SearchResult> = resps.into_iter()
        .zip(i_opts.queries.iter())
        .map(|(mut sr, query)| { 
            // filter out search results from each response
            sr.filter(query, 2);
            sr.pick_result(query)
        }).collect();

    let files = mbrew.files_from_results(&results, &i_opts.target).await.unwrap();
    ...
    mbrew.download_files(&files, &mods_folder).await.unwrap();
...
//     // Loop through every query made
//     // Turns quries into ModFile structs which have a download link
//     let resps = Search::new(&i_opts.queries, &i_opts.target);
//
//     let downloads: Vec<ModFile> = i_opts.queries.into_iter().map(|query| async {
//         println!("Searching modrinth for {} for Minecraft {}", query, &i_opts.target);
//         let s = Search::new(&query, &i_opts.target);
//         let mut results = s.search().await.unwrap();
//
...
//         let version = Version::search(&res.slug, &i_opts.target).await.unwrap().remove(0);
